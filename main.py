...
def get_ginger_result(text):
    ...
    try:
        ...
        st.error("Value Error: Invalid server response.")
        return None
...
def check_grammar(text):
    ...
    if len(original_text) > 600:
        return "You can't check more than 600 characters at a time."
    ...
    for result in results["LightGingerTheTextResult"]:
        if result["Suggestions"]:
            ...
            fixed_text = fixed_text[:from_index] + suggest + fixed_text[to_index:]
